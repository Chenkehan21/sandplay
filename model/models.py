...
class KNN(nn.Module):
    def __init__(self, num_classes=6) -> None:
        ...
        clip_model, _ = clip.load(clip_model_name)
        self.img_encode = clip_model
        self.knn = KNeighborsClassifier(n_neighbors=num_classes)
    
    def forward(self, imgs):
        features = self.img_encode(imgs)
        y_pred = self.knn.predict(features.numpy())
        
        return torch.from_numpy(y_pred)
    
    def fit(self, x, y):
        features = self.img_encode(x)
        self.knn.fit(features.numpy(), y.numpy())
